...
class WatchRegistry:
    """
    Single platform observer, many watched dirs.
    ...
    def __init__(self):
        ...
        log.info("\n" + block(f"OBSERVER STARTED ({backend_name(self.ObserverClass)})"))
    ...
    def enable(self, sid: str, abs_path: str, cb: Callable[[dict], None], recursive: bool = True):
        ...
        try:
            ...
            # Linux: inotify ENOSPC; other OS raise different errors but still bubble up
            if getattr(e, "errno", None) in (28,) or "inotify" in str(e).lower():
                log.error("\n" + block("WATCH ENABLE FAILED (SYSTEM LIMIT)", sid=sid, path=abs_path))
                raise RuntimeError(
                    "Failed to enable watch (system watch limit). Adjust OS limits and retry."
                ) from e
            raise
        except Exception:
            log.exception("watch schedule failed", extra={"sid": sid, "path": abs_path})
            raise
